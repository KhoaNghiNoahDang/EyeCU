...
import { Pill, Syringe, FileWarning, Clock } from "lucide-react";
    ...
    <div className="p-4 space-y-4 bg-slate-50 min-h-full">
        {extractedMedications ? (
          <>
            ...
            {extractedMedications.map((med: any, i: number) => (
              <div key={i} className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm">
                <div className="flex items-start gap-3 border-b border-slate-100 pb-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-[16px] font-bold text-[#0d1f2d]">{med.name}</h3>
                    <p className="text-sm font-medium text-slate-500 mt-0.5">Số lượng: <span className="text-[#0d1f2d]">{med.quantity}</span></p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[13px] text-slate-600">{med.dosage}</p>
                  </div>
                </div>
              </div>
            ))}
